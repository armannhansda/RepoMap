...
// ../parser/src/callExtractor.ts
var import_fs3 = __toESM(require("fs"), 1);

// ../parser/node_modules/glob/dist/esm/index.min.js
...
// ../parser/src/scanner.ts
var import_promises2 = require("node:fs/promises");
var import_node_path2 = __toESM(require("node:path"), 1);
async function scannerRepository(repoPath, cachedFiles) {
  if (cachedFiles) {
    return cachedFiles;
  }
  const matches = await Ze(
  ...
  return matches.map((file) => ({
    absolutePath: file,
    relativePath: import_node_path2.default.relative(repoPath, file)
  }));
}
async function scanAndCacheRepository(repoPath) {
  const files = await scannerRepository(repoPath);
  const cached = await Promise.all(
    files.map(async (file) => {
      try {
        const content = await (0, import_promises2.readFile)(file.absolutePath, "utf-8");
        return {
          ...file,
          content
        };
...
// ../parser/src/functionExtractor.ts
var import_fs2 = __toESM(require("fs"), 1);

// ../parser/src/findEndLine.ts
function findEndLine(content, startMatchIndex, isPython) {
...
async function extractFunctions(repoPath, cachedFiles) {
  const files = await scannerRepository(repoPath, cachedFiles);
  const functions = [];
  for (const file of files) {
    const content = file.content !== void 0 ? file.content : import_fs2.default.readFileSync(file.absolutePath, "utf-8");
...
// ../parser/src/callExtractor.ts
var CALL_REGEX = /\b([a-zA-Z0-9_]+)\s*\(/g;
async function extractCalls(repoPath, functions, cachedFiles) {
  if (!functions) {
    functions = await extractFunctions(repoPath, cachedFiles);
  }
  const files = await scannerRepository(repoPath, cachedFiles);
  const calls = [];
  const fileFunctions = /* @__PURE__ */ new Map();
  for (const fn of functions) {
    if (!fileFunctions.has(fn.file)) fileFunctions.set(fn.file, []);
    fileFunctions.get(fn.file).push(fn);
  }
  for (const file of files) {
    const content = file.content !== void 0 ? file.content : import_fs3.default.readFileSync(file.absolutePath, "utf-8");
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      let match;
      const re2 = new RegExp(CALL_REGEX);
...
// ../parser/src/graphBuilder.ts
function buildGraph(dependencies, functions, calls, classes = [], interfaces = [], techStack) {
  const nodes = [];
  const edges = [];
  const importedByMap = /* @__PURE__ */ new Map();
  const functionLookup = /* @__PURE__ */ new Map();
  const fileFunction = /* @__PURE__ */ new Map();
  const fileClass = /* @__PURE__ */ new Map();
  const fileInterface = /* @__PURE__ */ new Map();
  const functionCalls = /* @__PURE__ */ new Map();
  const calledByMap = /* @__PURE__ */ new Map();
  ...
  const functionByFileAndName = /* @__PURE__ */ new Map();
...
// ../parser/src/importExtractor.ts
var import_fs5 = __toESM(require("fs"), 1);

// ../parser/src/resolveImports.ts
...
function resolveImportPath(currentPath, importPath, repoPath) {
  if (!importPath.startsWith(".")) {
    return null;
  }
  ;
  const currentDir = import_path.default.dirname(currentPath);
  const absoluteImportPath = import_path.default.resolve(currentDir, importPath);
  if (import_fs4.default.existsSync(absoluteImportPath) && import_fs4.default.statSync(absoluteImportPath).isFile()) {
    return import_path.default.relative(repoPath, absoluteImportPath);
  }
  for (const ext of extension) {
    const fullPath = absoluteImportPath + ext;
    if (import_fs4.default.existsSync(fullPath)) {
      return import_path.default.relative(
        repoPath,
        fullPath
      );
    }
  ...
  for (const ext of extension) {
    const indexPath = import_path.default.join(absoluteImportPath, `index${ext}`);
    if (import_fs4.default.existsSync(indexPath)) {
      return import_path.default.relative(repoPath, indexPath);
    }
  }
  return null;
}
...
async function extractImports(repoPath, cachedFiles) {
  const files = await scannerRepository(repoPath, cachedFiles);
  const dependencyMap = [];
  for (const file of files) {
    const content = file.content !== void 0 ? file.content : import_fs5.default.readFileSync(file.absolutePath, "utf-8");
    ...
    for (const regex of IMPORT_REGEXES) {
      ...
      while ((match = re2.exec(content)) !== null) {
        ...
        if (!resolvedPath && !importVal.startsWith(".")) {
          ...
          const absoluteAsPath = import_path2.default.resolve(repoPath, asPath);
          for (const ext of [".py", ".java"]) {
            if (import_fs5.default.existsSync(absoluteAsPath + ext) && import_fs5.default.statSync(absoluteAsPath + ext).isFile()) {
              resolvedPath = import_path2.default.relative(repoPath, absoluteAsPath + ext);
              break;
            }
          }
        }
...
async function extractClasses(repoPath, cachedFiles) {
  const files = await scannerRepository(repoPath, cachedFiles);
  const classes = [];
  for (const file of files) {
    const content = file.content !== void 0 ? file.content : import_fs6.default.readFileSync(file.absolutePath, "utf-8");
    const isPython = file.absolutePath.endsWith(".py");
    const foundNames = /* @__PURE__ */ new Set();
    for (const regex of CLASS_REGEXES) {
      ...
      while ((match = re2.exec(content)) !== null) {
        ...
        const matchStr = match[0] || "";
        const isExported = matchStr.includes("export ") || matchStr.includes("pub ") || matchStr.includes("public ");
        const extendsClass = match[2] ? match[2].trim() : void 0;
        const implementsStr = match[3] ? match[3].split(",").map((s) => s.trim()).filter(Boolean) : void 0;
        classes.push({
          name,
          file: file.relativePath,
          type: "class",
...
async function extractInterfaces(repoPath, cachedFiles) {
  const files = await scannerRepository(repoPath, cachedFiles);
  const interfaces = [];
  for (const file of files) {
    const content = file.content !== void 0 ? file.content : import_fs6.default.readFileSync(file.absolutePath, "utf-8");
...
async function extractApiEndpoints(repoPath, functions, cachedFiles) {
  const files = await scannerRepository(repoPath, cachedFiles);
  for (const fn of functions) {
  ...
  for (const file of files) {
    const content = file.content !== void 0 ? file.content : import_fs7.default.readFileSync(file.absolutePath, "utf-8");
    const lines = content.split("\n");
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      const lineNumber = i + 1;
...
// ../parser/src/techStackExtractor.ts
...
async function extractTechStack(repoPath, cachedFiles) {
  const files = await scannerRepository(repoPath, cachedFiles);
  const languagesSet = /* @__PURE__ */ new Set();
  const frameworksSet = /* @__PURE__ */ new Set();
  let packageCount = 0;
  for (const file of files) {
    const ext = import_path3.default.extname(file.relativePath);
    if (EXT_TO_LANG[ext]) {
      languagesSet.add(EXT_TO_LANG[ext]);
    }
  }
  const packageJsonPath = import_path3.default.join(repoPath, "package.json");
  if (import_fs8.default.existsSync(packageJsonPath)) {
    try {
      const pkg = JSON.parse(import_fs8.default.readFileSync(packageJsonPath, "utf-8"));
      const allDeps = {
        ...pkg.dependencies || {},
        ...pkg.devDependencies || {}
      };
  ...
  const reqPath = import_path3.default.join(repoPath, "requirements.txt");
  if (import_fs8.default.existsSync(reqPath)) {
    try {
      const content = import_fs8.default.readFileSync(reqPath, "utf-8");
  ...
  const goModPath = import_path3.default.join(repoPath, "go.mod");
  if (import_fs8.default.existsSync(goModPath)) {
    try {
      const content = import_fs8.default.readFileSync(goModPath, "utf-8");
...
async function parseRepository(repoPath) {
  ...
  const [_2, calls] = await Promise.all([
  ...
  let disposable = vscode.commands.registerCommand("repomap.start", async () => {
    ...
    const panel = vscode.window.createWebviewPanel(
      ...
      vscode.ViewColumn.One,
      {
        ...
        localResourceRoots: [
          vscode.Uri.file(path5.join(context.extensionPath, "..", "client", "out"))
        ]
      }
    );
      ...
      async (message) => {
        if (message.type === "OPEN_FILE") {
          if (message.file) {
            ...
            const fullPath = path5.join(rootPath, message.file);
        ...
        if (message.type === "API_REQUEST") {
          ...
          try {
            ...
            if (message.endpoint === "analyzeRepo") {
              ...
              const fullPath = path5.join(rootPath, message.payload.filePath);
...
function getWebviewContent(webview, extensionPath) {
  const outPath = path5.join(extensionPath, "..", "client", "out");
  const htmlPath = path5.join(outPath, "index.html");
